"""
Similar to compute_simulated_le_stats.py, but for Lambda_1 (the version that
conditions the frequency of the B allele).

This script also takes in npy files rather than the raw .txt.gz output of 
the simulations. gz_freq_arr_to_npy.py is used to convert the raw output to npy
