"""
This script is used to estimate Lambda scaling for the microbiome data.
It takes in the haplotype count table (stored in a .npy format for speed) and
uses estimators.py to calculate the Lambda scaling for different f0s and ell ranges.

The code first calculates a frequency scan for a few ell ranges, and then calculates
a distance scan for a few f0s. The results are saved in the savepath directory.

The saved npy files include the frequencies & ells used for the scan, and the
estimated Lambda values for each frequency & ell combination. These files were
used to generate the microbiome plots in the manuscript.
"""
