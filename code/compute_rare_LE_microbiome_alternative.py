...
for idx in range(0,len(ellranges)):
    ...
    for f0 in f0s:
        ...
        # now a few alternative stats
        alt_numer = estimator.calculate_LE_numer_alternative(n_obs[good_idxs, :], f0, num_reps=200)
        alt_denom1 = estimator.calculate_LE_denom_single_site_moment(nAs[good_idxs], ntots[good_idxs], f0)
        alt_denom2 = estimator.calculate_LE_denom_single_site_indicator(nAs[good_idxs], ntots[good_idxs], f0, num_reps=200)
        all_alt_numers.append(alt_numer)
        all_alt_denoms1.append(alt_denom1)
        all_alt_denoms2.append(alt_denom2)

    sigmasquareds = numpy.array(sigmasquareds)
    all_numers = numpy.array(all_numers)
    all_denoms = numpy.array(all_denoms)
    all_alt_numers = numpy.array(all_alt_numers)
    all_alt_denoms1 = numpy.array(all_alt_denoms1)
    all_alt_denoms2 = numpy.array(all_alt_denoms2)
    ...
    numpy.save(os.path.join(savepath, "f_scan_stats_alt_numer_{}".format(idx)), all_alt_numers)
    numpy.save(os.path.join(savepath, "f_scan_stats_alt_denom1_{}".format(idx)), all_alt_denoms1)
    numpy.save(os.path.join(savepath, "f_scan_stats_alt_denom2_{}".format(idx)), all_alt_denoms2)
...
"""
skipping ell scan for testing purposes
