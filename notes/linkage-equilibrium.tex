...
\begin{align}
    z_1(u) = \alpha e^{-u} \int_0^u e^{u'} f_1(u') du' - e^{-u} \int_0^u e^{u'} z_0^2 (u') du'.
\end{align}
...
%\end{align}
If $\gamma_A, \gamma_B \gg 1$, then the first two terms in Eq. (\ref{eq:f(u)_series}) can be found as 
% by Taylor expanding the denominators
\begin{align}
    f_0(u) &= x e^{-\beta_A u} + y e^{-\beta_B u}
\end{align}
and
\begin{align}
    f_1(u) &= -x^2 \frac{1}{\beta_A}e^{-\beta_A u} \left(1-e^{-\beta_Au}\right)
...
To the first order in $\epsilon$, the the numerator of $\Lambda(f_0)$, then follows as
...
\end{align}\end{subequations}
into Eq. (\ref{eq:lambda_num_large_rho}) and evaluating the integrals, we obtain
\begin{align}
        \left\langle f_{Ab}f_{aB}f_{AB}\cdot e^{-\frac{f_{A}+f_{B}}{f_0}}\right\rangle 
        &\approx \frac{\epsilon^2\alpha\theta^2f_0^4}{\beta_A^2\beta_B^2(1+\beta_A+\beta_B)} \\\nonumber
        & \times \left[1 +\left(1+\frac{\alpha}{\beta_A}\right)\frac{2\beta_A^2}{(1+\beta_B)(2+\beta_B)}
        +\left(1+\frac{\alpha}{\beta_B}\right)\frac{2\beta_B^2}{(1+\beta_A)(2+\beta_A)}\right]
\end{align}
to the lowest order in $\epsilon$.
